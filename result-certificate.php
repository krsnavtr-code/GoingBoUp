...
if ($_SESSION['role'] == "docs-verification" || $_SESSION['role'] == "super-admin") {
	...
	<div class="content-wrapper">
		...
		<section class="content">
					...
					<form action="" method="post">
						<div class="row">
							<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
								<input type="text" placeholder="Search : enrolment,course, Branch" class="form-control"
									name="valueToSearch">
							</div>
							<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
								<input type="submit" placeholder="submit" name="search"
			...
			<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"
					style="overflow-x:auto; margin-bottom: 20px; height:530px;">
							...
							if (!empty($row['certificate_month'] && $row['certificated_deliver_date'])) {
								...
								<tr>
									<?php if ($row['DownloadResult'] == 1) { ?>
										<td>
											<button
												onclick="printcfunction(<?php echo "'" . $row['enrolment'] . "'" . "," . "'" . $row['semester'] . "'" . "," . "'" . $prdata['course'] . "'" . "," . "'" . $row['branch'] . "'"; ?>)"
												class="btn btn-block btn-success btn-xs ActionButtonRa"> Print Certificate</button>
										</td>
									<?php } else { ?>
										<td>
			...
			function printcfunction(enrollement, semester, course, branch) {
				$.ajax({
					...
					success: function (data) {
						...
						// 		"path": ["result-print.css"]
						// 	});
						// }, 500); // 500ms ka delay CSS load hone ke liye
					}
				});
			}
		</script>
